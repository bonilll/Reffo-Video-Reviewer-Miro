...
>(({ className, sideOffset = 4, ...props }, ref) => {
  ...
  // Gestione degli attributi per l'accessibilità
  React.useEffect(() => {
    if (isMounted && contentRef.current) {
      // Rimuovi aria-hidden se presente
      if (contentRef.current.hasAttribute('aria-hidden')) {
        contentRef.current.removeAttribute('aria-hidden');
      }
      
      // Imposta l'attributo data-portal per lo styling
      contentRef.current.setAttribute('data-portal', 'true');
      
      return () => {
        if (contentRef.current) {
          contentRef.current.removeAttribute('data-portal');
        }
      };
    }
  }, [isMounted]);
